...
void UpdateChartStatusPanel()
{
   ...
   if(g_currentState >= STATE_FIB_ACTIVE && g_currentState <= STATE_IN_POSITION)
   {
      ...
      int totalTouch = g_touchCount_Primary + g_touchCount_Deep;
      PanelSetLine(r++, "Touch    : " + IntegerToString(totalTouch) +
                        " (P:" + IntegerToString(g_touchCount_Primary) +
                        " D:" + IntegerToString(g_touchCount_Deep) + ")");
      PanelSetLine(r++, "DeepBand : " + (g_goldDeepBandON ? "ON" : "OFF"),
                   g_goldDeepBandON ? clrYellow : PANEL_CLR_DIM);
      PanelSetLine(r++, "Band(P)  : " + DoubleToString(g_primaryBandLower, digits) +
